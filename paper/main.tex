...
Fast improvement and widespread accessibility of Large Language Models (LLMs) have destroyed the lines between human-written and machine-generated text. While this technology offers benefits, it also presents significant disadvantages, including academic dishonesty, disinformation, and the fall of trust in digital content [xu2024detecting]https://doi.org/10.1609/aaai.v38i21.30361, [hans2024spotting]https://github.com/ahans30/Binoculars. This has created an inevitable need for robust methods to detect AI-generated content.

Current detectors often rely on fine-tuned classifiers or statistical signatures taken from the text [gehrmann2019gltr]https://arxiv.org/abs/1906.04043. Useful and computationally efficient feature is perplexity, a measure of how "surprising" a text is to a language model. The assumption is that LLMs, which are trained to maximize likelihood, will generate text with lower perplexity compared to human writing. As it is shown, a significant change comes from distribution shift. The perplexity of a text is not an absolute value; it is highly dependent on the domain (e.g., news, scientific articles, social media) and language it was trained on. A model fine-tuned for one genre often fails on another, causing unreliable detection in real world, where the target domain is unknown [bakhteev2022automatic]https://ieeexplore.ieee.org/abstract/document/9983964.

Recent works have attempted to solve this by developing zero-shot detectors that are more robust to unseen prompts or models. For instance, Binoculars [hans2024spotting]https://github.com/ahans30/Binoculars uses a ratio of scores from two different LLMs for calibration. Similarly, research on multilingual and cross-domain detection highlights the necessity of adapting methods to diverse data distributions [gritsay2023artificially]https://doi.org/10.1134/S1064562423701211. Despite these advances, a method for calibrating perplexity itself to make it robust across different genres and languages remains still not-explored.
