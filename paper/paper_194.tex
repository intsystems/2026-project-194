...
$\mathcal{M}_d$ is n-gram model providing probabilities of tokens
...
\begin{enumerate}
    ...
    \item \textbf{Ensemble inference:} For a new text, compute perplexities from all domain models and combine their predictions using one of three ensemble strategies.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Validation set $\mathbf{V}_d \subset \mathbb{D}$ with balanced human and machine texts from domain $d$
    \item Test set $\mathbf{X}_d \subset \mathbb{D}$ held-out from the same distribution as validation
\end{itemize}
...
\subsubsection{Strategy A: Simple Majority Voting}
...
\subsubsection{Strategy B: Native Domain Detection}

This strategy is based on the observation that a text originating from a particular domain yields minimal perplexity for the model trained on that domain. We first identify the \textit{native domain} of the text:
\[
...
\subsubsection{Strategy C: Weighted Voting by Domain Significance}

We assign weights to each model based on its ability to distinguish between human and machine texts. The weight $w_d$ for domain $d$ is proportional to the difference between the model's true positive rate and false positive rate on the validation set:
\[
w_d = \frac{\text{TPR}_d - \text{FPR}_d}{\sum_{d' \in \mathcal{D}} (\text{TPR}_{d'} - \text{FPR}_{d'})},
\]
where $\text{TPR}_d$ is the true positive rate (recall on machine texts) and $\text{FPR}_d$ is the false positive rate (errors on human texts) for model $\mathcal{M}_d$. Models that better distinguish between classes receive higher weights.
...
\begin{itemize}
    ...
    \item \textbf{Scalability:} Adding a new domain requires training one additional KenLM model and computing its validation statistics.
\end{itemize}
...
\[
\mathcal{L} = \sum_{i < j} \left( \mathbf{1}[p_{d_i} < \tau_{d_i}] - \mathbf{1}[p_{d_j} < \tau_{d_j}] \right)^2 \rightarrow \min.
\]
...
\begin{table}[H]
\centering
\caption{Optimal thresholds and F1-scores for each domain (in-domain evaluation).}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Domain} & \textbf{Optimal threshold} $\tau_d$ & \textbf{F1-score} \\
\hline
...
\hline
\textbf{Method} & \textbf{Acc} & \textbf{F1} & \textbf{Human recall} & \textbf{AI recall} \\
\hline
...
\textbf{Observations:}
\begin{itemize}
    \item \textbf{Strategy A (Simple majority voting)}: A text is classified as machine-generated if at least one model votes for machine. This achieves 79.1\% human recall and 72.2\% AI recall.
    \item \textbf{Strategy B (Native domain detection)}: For each text, we identify the model with minimal perplexity (the ``native'' domain) and use its individual prediction. This yields the highest human recall (82.7\%) with AI recall of 69.0\%.
    \item \textbf{Strategy C (Weighted voting)}: Weights are assigned based on each model's discriminative power (TPR - FPR) on the validation set. This method achieves balanced performance with 79.2\% human recall and 71.8\% AI recall.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Domain shift is significant:} Optimal thresholds vary substantially across domains, ranging from 8.28 (AINL science) to 12.10 (hard news).
    \item \textbf{Single-domain models do not generalize:} Cross-domain performance drops significantly compared to in-domain evaluation.
    \item \textbf{Ensemble methods balance the trade-off:} All three ensemble strategies achieve 0.76 accuracy and 0.75 F1-score, with different class-specific biases.
    \item \textbf{Native domain detection excels at human recall:} Identifying the domain with minimal perplexity and using its prediction achieves 82.7\% human recall.
    \item \textbf{Simple majority voting provides balanced performance:} With 79.1\% human recall and 72.2\% AI recall, this is the simplest and most robust approach.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Simple majority voting} achieves balanced performance (79\% human recall, 72\% AI recall) with 0.76 accuracy.
    \item \textbf{Native domain detection} (using the model with minimal perplexity) yields the highest human recall (83\%) at 69\% AI recall.
    \item \textbf{Weighted voting} provides similar balanced results.
\end{itemize}
...
\textbf{Limitations:} Our work is limited to Russian language, seven domains, and KenLM as the base model. Results may not directly generalize to other languages or neural models.

\textbf{Future work:} Comparing with other models,  adaptive threshold selection for unseen domains.
...
\begin{enumerate}
    \item Optimal perplexity thresholds vary significantly across domains (8.28–12.10), confirming domain dependence.
    \item Three ensemble strategies combining domain-specific KenLM models achieve robust detection without knowledge of the test text's domain.
    \item Simple majority voting achieves 79\% human recall and 72\% AI recall (accuracy 0.76, F1 0.75).
    \item Native domain detection achieves the highest human recall (83\%).
\end{enumerate}
